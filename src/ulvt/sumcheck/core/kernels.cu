...
__global__ void fold_large_list_halves(
	...
	// print contents of device_matrix_rows_height_7
	// printf("fold_large_list_halves: %d\n", TOWER_HEIGHT);
	// for (int i = 0; i < 128; ++i) {
	// 	printf("Row %d: %016llx\n", i, (unsigned long long)(device_matrix_rows_height_7[i][0]));
	// 	printf("Row %d: %016llx\n", i, (unsigned long long)(device_matrix_rows_height_7[i][1] ));
	// 	printf("\n");
	// }
	...
	for (uint32_t row_idx = tid; row_idx < num_batch_rows; row_idx += gridDim.x * blockDim.x) {
		for (uint32_t col_idx = 0; col_idx < num_cols; ++col_idx) {
			...
			fold_batch(lower_batch, upper_batch, dst_batch, coefficient);
		}
	}
}
