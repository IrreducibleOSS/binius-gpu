                                      ...
                                      uint32_t Z[128] )
{
	...
	// {
	// 	if (device_matrix_rows_height_7[k] == 0) {
    //     printf("Row %llu is zero, skipping\n", static_cast<unsigned long long>(k));
    ...
    for (int i = 0; i < OUTER_LOOP; ++i)
    {
        ...
        for (int j = 0; j < 128; ++j)
        {
            uint8_t* curren_lookup_rows_bytes = (uint8_t*)&device_matrix_rows_height_7[j][0];
...
__host__ __device__ void fold_batch(
	...
	// printf("fold_batchingggggggggggggggggggg: %d\n", TOWER_HEIGHT);
	// for (int i = 0; i < 128; ++i) {
	// 	printf("Row %d: %016llx\n", i, (unsigned long long)(device_matrix_rows_height_7[i] >> 64));
	// 	printf("Row %d: %016llx\n", i, (unsigned long long)(device_matrix_rows_height_7[i] & 0xFFFFFFFFFFFFFFFF));
	// 	printf("\n");	
	// }
